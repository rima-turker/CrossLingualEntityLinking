...
public class CompareGTWithYovisto 
{
	/***
	 * 
	 ...
	 */
	private static final int TOP_NCandidates = Config.getInt("TOPN_CANDIDATES", 0);
	...
	public void testAnchorLinkDictionary() 
	{
		...
		for (Tuple touple: gt) 
		{
			...
			if (dictionary.containsKey(anchor))
			{
				...
				final Map<String, Double> mapFirstNCandidates = new LinkedHashMap<String, Double>(
						MapUtil.getFirstNElement(MapUtil.sortByValueDescending(dictionary.get(anchor)), TOP_NCandidates));
		...
		TOP_NCandidates+" "+lstTop.size()+", entities lower case "+countFound_lowerCase+
	...
	public void compareGTWithYovisto()
	{
		EDBanchmark_DataExtraction dataClean = new EDBanchmark_DataExtraction();
		...
		Map <Integer,Integer> mapTopNStatistic = new HashMap<>();
		...
		int countNoCandidateFoundButTagged = 0;
		...
		for (Entry<String, List<Tuple>> entry: mapAnchor.entrySet()) 
		{
			...
			List<Tuple> lstAnchorsAndGTs = entry.getValue();
					...
					for(Tuple touple:lstTagged)
					{
						...
						if (touple.getA().equals(anchor)) 
						{
							...
							if (candidates.contains(gt))
							{
								...
								int findTopN = findTopN(candidates,gt);
								if (findTopN<=TOP_NCandidates&&findTopN>0) {
									inTop++;
								}
								if (mapTopNStatistic.containsKey(findTopN)) 
								{
									int tempCount = mapTopNStatistic.get(findTopN);
									mapTopNStatistic.put(findTopN, tempCount+1);
								}
								else
								{
									mapTopNStatistic.put(findTopN,1);
								}
								//System.out.println(anchor+"\t"+gt+"\t"+findTopN);
								//System.out.println("YES");
							}
							else
							{
								countNoCandidateFoundButTagged++;
...
//					}
					if (!taggedAnchor) {
						...
						countNotTagged+=1;
						//System.out.println(mapContext.get(entry.getKey()));
					}
				}
		...
		System.out.println("totalAnchorCount "+ totalAnchorCount+ ", countCandidateFound "+ countCandidateFound
				+", countNoCandidateFoundButTagged "+countNoCandidateFoundButTagged+ " countNotTagged, "+countNotTagged);
		System.out.println("If anchor is not tagged you cannot easliy generate candidates, otherwise not possible");
		mapTopNStatistic.forEach((k,v)-> System.out.println(k+", "+v));
		System.out.println("In top "+TOP_NCandidates+" "+inTop+" entities found");
